_Dernière compilation : 2026-08-01 · 20 articles publiés · DataForSEO **stable** (cache réutilisé, expansion non nécessaire) · GSC **toujours cassé** (`deleted_client`, inchangé, 5ᵉ run) · Reddit **toujours en 403** (inchangé)._

## Résumé des chiffres et analyse

**Production de contenu**
- **20 articles publiés** entre le 2026-05-20 et le 2026-08-01 (8 en FR, 10 en EN, 1 en ES, 1 en DE).
- **1 nouvel article le 01/08.** Le run a de nouveau utilisé le cache `candidates.json` (139 mots-clés non utilisés, pas d'expansion DataForSEO nécessaire).
- ⚠️ **Le pick automatique du run du 01/08 a de nouveau été écarté — 3ᵉ collision consécutive sur la même paire de mots-clés** (après le 21/07 et le 25/07) : le pipeline a recommandé pour la troisième fois `how analyse qualitative data` (score 24, pillar `recherche-qualitative`), quasi-doublon de `analyse-qualitative-data.md` (les 4 questions PAA du run — méthodes, ChatGPT, démarche, exemples — y sont déjà toutes traitées mot pour mot). Le check de dédoublonnage exact sur `targetKeyword` n'a toujours pas été corrigé (voir « À faire », priorité renforcée). **Publié à la place** : `best software to analyse qualitative data` (vol 10, KD 0, score 1.26, intent commercial), le candidat comparatif logiciels QDA (NVivo, ATLAS.ti, MAXQDA, Delve, Dovetail, Taguette) déjà identifié dans les « Propositions pour la suite » et les « À faire » du rapport précédent comme angle non exploité du cluster. Décision éditoriale tranchée en publiant : HumanLogs n'est pas un outil QDA, l'article le positionne comme l'étape de transcription en amont de ces logiciels (formats d'export compatibles), ce qui répond à l'item « À faire » correspondant sans disparaging des concurrents.
- **GSC toujours cassé** : même erreur `deleted_client` (HTTP 401) — le client OAuth n'a toujours pas été recréé (voir « À faire », inchangé depuis le 13/07, 5ᵉ run consécutif en échec).
- **Reddit toujours en échec 403** sur les 8 subreddits (`old.reddit.com/r/<subreddit>/search`) — inchangé depuis le 21/07, voir « À faire ». **0 posts** remontés ce run (comme attendu vu le blocage).
...
- `recherche-qualitative` (productFit 0.9) : **9 articles** — pillar le plus couvert (FR + EN + ES + DE sur l'angle « analyser des données qualitatives », un angle « exemple concret d'analyse », un angle « données de questionnaire », un angle « données d'entretien », et désormais un angle « comparatif logiciels », intent commercial). Tête de cluster à haut volume (`analyse qualitative` : 2400/mo, score 216) déjà traitée.
...
**Analyse**
- Le pillar `recherche-qualitative` passe à **9 articles** ; l'angle « analyser des données qualitatives » (5 variantes EN/FR/ES/DE + « exemple » + « questionnaire » + « entretien ») reste saturé, mais le nouvel article **change de registre** (comparatif logiciels, intent commercial) plutôt que d'ajouter une variante de plus. Les prochains articles du pillar devraient continuer sur des sous-thèmes distincts (`codification entretien`, `analyse verbatim`) plutôt que de nouvelles variantes de « comment analyser ».
- ⚠️ **Angle mort du dédoublonnage reproduit une troisième fois** : le pick automatique du 01/08 était de nouveau `how analyse qualitative data`, le même quasi-doublon identifié le 21/07 et le 25/07. Le correctif d'ingénierie proposé (`pipeline/select-topic.ts`, matching flou) n'a toujours pas été appliqué, donc le check exact sur `targetKeyword` laisse toujours passer cette variante à un mot près. Écarté manuellement une troisième fois, cette fois au profit de `best software to analyse qualitative data` — un candidat à intent commercial et angle différencié (logiciels), pas juste une reformulation de la question déjà traitée, ce qui limite le risque de retomber sur la même collision au prochain run. **Le contournement manuel n'est plus soutenable indéfiniment sur ce pillar** : les variantes restantes de « how do you/we/i analyse » cannibalisent toutes fortement l'existant (voir Propositions ci-dessous) — le correctif de dédoublonnage devient bloquant pour la suite du cluster.
...
- **DataForSEO stable** : cache `candidates.json` toujours frais (139 mots-clés non utilisés), aucune expansion nécessaire ce run, donc pas de nouveau coût engagé.
- **GSC toujours en échec** (5ᵉ run consécutif) : `"error": "deleted_client", "error_description": "The OAuth client was deleted."` (HTTP 401) — le client OAuth n'a toujours pas été recréé.
...
- **Ralentir sur `recherche-qualitative`** côté angle « analyser des données » (7 variantes en comptant questionnaire et entretien, saturé) et pivoter vers des sous-thèmes distincts du même pillar (`codification entretien`, `analyse verbatim`) pour continuer à exploiter son productFit sans cannibaliser. L'angle « logiciel » vient d'être ouvert (01/08) — envisager une déclinaison FR (`logiciel analyse qualitative`, à créer côté seeds) plutôt qu'une nouvelle variante EN du même comparatif.
...
- **Fiabiliser le dédoublonnage** (`pipeline/select-topic.ts`) : ajouter un matching flou (normalisation + distance d'édition ou similarité de tokens) en plus du match exact sur `targetKeyword` — priorité maximale après une **troisième** collision consécutive (21/07, 25/07, 01/08) sur la même paire de mots-clés quasi identiques ; les variantes restantes (« how do you/we/i analyse... ») garantissent une 4ᵉ collision si non corrigé avant le prochain run.
- **Maillage interne** entre les 9 articles du cluster `recherche-qualitative` (EN/FR/ES/DE) et vers les articles `transcription` — le nouvel article logiciels devrait aussi lier vers `ai-transcription-tools-research-compared.md` (comparatif adjacent, outils de transcription plutôt que d'analyse).

## Propositions pour la suite

Top candidats **non encore publiés**, issus de `pipeline/out/morning-report.md` (run du 2026-08-01) :
...
Note : la table `morning-report.md` du 01/08 remonte plusieurs mots-clés **déjà publiés** avec un score recalculé (`data analysis in qualitative research example`, `how to analyse qualitative interview data`, `how to analyse qualitative survey data`, `transcribe audio to text free online`, `mp3 audio to text converter online free` — le dédoublonnage exact les a laissés remonter car DataForSEO les rescore sans que le pipeline filtre par ces variantes). **Ne pas les republier** — comptés ici uniquement pour mémoire, retirer manuellement avant de piocher.
...
- `best software to analyse qualitative data` a été **publié le 01/08** (voir table de production) — retiré de cette liste. `software used to analyse qualitative data` (même intent commercial, même score) reste un quasi-doublon du nouvel article : ne pas le republier séparément, au mieux l'ajouter comme variante secondaire dans le `targetKeyword`/tags existants si le suivi de position le justifie.
- Les 6 requêtes FR à faible volume (`recherche qualitative`, `recherche qualitative et quantitative`, `méthodes de recherche qualitative`, `recherche qualitative définition`, `exemple de recherche qualitative`, `introduction à la recherche qualitative`) forment un bon socle pour une **page pilier FR « qu'est-ce que la recherche qualitative »**, plus large que les articles actuels centrés sur l'analyse — à regrouper en un seul article plutôt que 6 variantes séparées vu le volume très faible de chacune.
...
- [ ] ⚠️ **Fiabiliser le dédoublonnage du pipeline** (`pipeline/select-topic.ts`) — **priorité maximale, 3ᵉ occurrence** : le run du 01/08 a recommandé pour la troisième fois le même quasi-doublon (`how analyse qualitative data` vs `how to analyse qualitative data` déjà publié le 27/06), déjà signalé le 21/07 et le 25/07, jamais corrigé depuis. Trois collisions consécutives sur la même paire de mots-clés confirment que ce n'est pas un cas isolé, et les variantes restantes (« how do you/we/i analyse ») rendent une 4ᵉ collision quasi certaine au prochain run. Ajouter une comparaison floue (normalisation, distance d'édition ou tokens communs) en plus du match exact sur `targetKeyword`.
