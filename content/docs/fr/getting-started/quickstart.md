...
## 2. Trois réglages avant de lancer
...
Lancez ensuite la transcription.

## 3. Passer à autre chose

Comptez environ dix minutes de traitement par heure d'audio. Vous pouvez fermer la page : le traitement se fait de notre côté et vous recevez un e-mail dès que la transcription est prête.

## 4. Corriger avec l'audio

Ouvrez le document : le lecteur audio en haut, la transcription en dessous, les locuteurs dans la colonne de gauche.

L'interaction à retenir : **cliquez sur un mot et l'audio se place à cet instant précis**. À partir de là :

- **Espace** ou **Tab** lit et met en pause. Dans le texte, utilisez **Alt/Ctrl + Espace**.
- **Maintenez Alt** pour la vitesse 0,5×, **Ctrl** pour 2×. Relâchez pour revenir à la normale.
- Cliquez sur un nom de locuteur pour le renommer dans tout le document : « Locuteur 1 » devient « Enquêteur » une bonne fois.

Les corrections sont enregistrées au fil de la frappe. Pas de bouton d'enregistrement, donc pas de version perdue.

Une première passe sur un entretien d'une heure prend en général 30 à 45 minutes. C'est le fait de travailler avec l'audio, et non contre lui, qui fait la différence.
...
- Vous travaillez sur des entretiens sensibles ? Activez le [chiffrement de bout en bout](/docs/privacy/encryption) avant le prochain dépôt.
- Plusieurs entretiens dans le même projet ? Regroupez-les dans une [étude](/docs/organize/studies).
- Vous voulez être rapide et pas seulement exact ? Apprenez les [raccourcis clavier](/docs/transcribe/keyboard-shortcuts).
